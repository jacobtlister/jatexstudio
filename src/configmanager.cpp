...
void ConfigManager::updateRecentFiles(bool alwaysRecreateMenuItems)
{
    ...
    if (alwaysRecreateMenuItems || (recentMenu->actions().count() != maxRecentFiles + maxRecentProjects + 4)) {
        ...
        QList<QKeySequence> openFirstSeqList = QList<QKeySequence>() << QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_T);
        const QList<QKeySequence>* openFirstPtr = &openFirstSeqList;
        newOrLostOldManagedAction(recentMenu, "firstFile", tr("Open first non-open file"), SLOT(fileOpenFirstNonOpen()), openFirstPtr);
...
void ConfigManager::updateUserMacroMenu()
{
    ...
    QList<QKeySequence> editMacrosSeqList = QList<QKeySequence>() << QKeySequence(Qt::Key_F5);
